  ...
  (lambda (senv var)
    (let loop ([senv senv]
      ...
      (cases static-environment senv
        ...
        [extend-senv (saved-vars exps saved-senv) (let loop2 ([saved-vars saved-vars]
                                                              [exps exps]
                                                              [position 0])
                                                    (if (null? saved-vars)
                                                        (loop saved-senv
...
(define (has-binding? senv var)
  (cases static-environment senv
    [empty-senv () #f]
    [extend-senv (saved-vars exps saved-senv) (or (memv var saved-vars)
  ...
  (let helper ([exp exp]
    ...
    (cases expression exp
      ...
      [unpack-exp (vars exp body) (helper body
                                          ...
                                          free-variables)]
      [letrec-exp (p-names b-vars p-bodies letrec-body) (let ([rec-senv (extend-senv* p-names senv)])
                                                          (helper letrec-body
                                                                  rec-senv
...
(define (inline-proc exp senv)
  (cases expression exp
    ...
    [unpack-exp (vars exp body) (unpack-exp vars
                                            ...
                                            (inline-proc body
                                                         (extend-senv* vars senv)))]
    [letrec-exp (p-names b-vars p-bodies letrec-body) (let ([rec-env (extend-senv-rec p-names senv)])
                                                        (letrec-exp p-names
                                                                    b-vars
  ...
  (lambda (exp nameless-env)
    (cases expression exp
      ...
      [nameless-letrec-var-exp (depth position) (let ([partial-proc (expval->proc (apply-nameless-env nameless-env
                                                  ...
                                                  (cases proc partial-proc
                                                    [procedure (body unused-env) (proc-val (procedure body
                                                                                                      real-env))]))]
      [nameless-let-exp (exps body) (let ([vals (map (lambda (exp)
      ...
      [nameless-unpack-exp (exp body) (let ([vals (expval->pack (value-of exp nameless-env))])
                                        (value-of body (extend-nameless-env vals nameless-env)))]
      [nameless-letrec-exp (p-bodies letrec-body) (let* ([make-proc (lambda (p-body)
