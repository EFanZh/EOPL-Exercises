...
(require "../solutions/exercise-7.5.rkt")
...
(check-equal? (run "let in 3") (num-val 3))
...
(check-equal? (run "let f = proc (x : int, y : int)
                              -(x, y)
                    in (f 7 4)")
              (num-val 3))

(check-equal? (run "letrec in 6") (num-val 6))

(check-equal? (run "letrec bool even?(x : int) = if zero?(x)
                                                 then zero?(0)
                                                 else (odd? -(x, 1))
                           bool odd?(x : int) = if zero?(x)
                                                   then zero?(1)
                                                   else (even? -(x, 1))
                    in (even? 7)")
              (bool-val #f))

(check-equal? (check "let in 3") 'int)
...
(check-equal? (check "proc ()
                        4")
              '(-> int))

(check-equal? (check "let f = proc ()
                                4
                      in (f)")
              'int)

(check-equal? (check "proc (x : int, y : int)
                        -(x, y)")
              '(int * int -> int))

(check-equal? (check "let f = proc (x : int, y : int)
                                -(x, y)
                      in (f 7 4)")
              'int)

(check-equal? (check "letrec in 6") 'int)

(check-equal? (check "letrec bool even?(x : int) = if zero?(x)
                                                   then zero?(0)
                                                   else (odd? -(x, 1))
                             bool odd?(x : int) = if zero?(x)
                                                     then zero?(1)
