...
(require rackunit)
(require "../solutions/exercise-5.40.rkt")
...
(check-equal? (run "try 2
                    catch (x c) x") (num-val 2))

(check-equal? (run "try raise 3
                    catch (x c) x") (num-val 3))

(check-equal? (run "try -(3, raise 5)
                    catch (x c) 5") (num-val 5))

(check-equal? (run "try try -(3, raise 5)
                        catch (x c) raise 7
                    catch (y c) y") (num-val 7))

(check-equal? (run "try 2
                    catch (x c) (c x)") (num-val 2))

(check-equal? (run "try raise 3
                    catch (x c) (c x)") (num-val 3))

(check-equal? (run "try -(3, raise 5)
                    catch (x c) (c 5)") (num-val -2))

(check-equal? (run "try try -(3, raise 5)
                        catch (x c) (c raise 7)
                    catch (y c) (c y)") (num-val -4))
