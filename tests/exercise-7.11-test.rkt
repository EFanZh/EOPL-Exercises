...
(require "../solutions/exercise-7.11.rkt")
...
(check-equal? (check "newpair(1, zero?(4))") '(mutpairof int bool))
(check-equal? (check "newpair(zero?(4), 1)") '(mutpairof bool int))
(check-equal? (check "left(newpair(zero?(4), 1))") 'bool)
(check-equal? (check "right(newpair(zero?(4), 1))") 'int)
(check-equal? (check "setleft newpair(zero?(4), 1) = zero?(0)") 'void)
(check-equal? (check "setright newpair(zero?(4), 1) = 4") 'void)
...
(check-equal? (check "let p = newpair(4, 6)
                      in begin setleft p = 1;
                               -(right(p), left(p))
                         end")
              'int)

(check-equal? (check "let p = newpair(4, 6)
                      in begin setright p = 1;
                               -(left(p), right(p))
                         end")
              'int)

(check-equal? (run "left(newpair(4, 6))") (num-val 4))
(check-equal? (run "right(newpair(4, 6))") (num-val 6))

(check-equal? (run "let p = newpair(4, 6)
                    in begin setleft p = 1;
                             -(right(p), left(p))
                       end")
              (num-val 5))
