...
(require rackunit)
(require "../solutions/exercise-5.54.rkt")
...
(check-equal? (run "let id1 = -1
                    in let mut = mutex()
                       in begin wait(mut);
                                let id2 = spawn(proc (id)
                                                  begin set id1 = id;
                                                        signal(mut)
                                                  end)
                                in begin wait(mut);
...
(let* ([output (with-output-to-string
       ...
       [nums (map string->number (string-split output))])
  (check-equal? nums '(100)))

(let* ([output (with-output-to-string
                 (λ ()
                   (run "let id1 = spawn(proc (id)
                                           print(100))
                         in begin kill(id1);
                                  print(200)
                            end")))]
       [nums (map string->number (string-split output))])
  (check-equal? nums '(200)))
