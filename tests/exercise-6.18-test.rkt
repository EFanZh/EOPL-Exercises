...
(require rackunit)
(require "../solutions/exercise-6.18.rkt")
...
(check-equal? (run "+(+(3, 4, 5), 6, 7)") (num-val 25))
(check-equal? (run "-(if zero?(0) then 4 else 3, 6)") (num-val -2))
(check-equal? (run "-(let x = 6 in x, 4)") (num-val 2))
(check-equal? (run "-(letrec f (x) = x in (f 7), 4)") (num-val 3))
