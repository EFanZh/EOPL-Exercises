...
(require "../solutions/exercise-7.28.rkt")
...
(check-equal? (check "pair(zero?(4), 3)") '(pairof bool * int))
(check-equal? (check "unpair a b = pair(zero?(4), 3) in a") 'bool)
(check-equal? (check "unpair a b = pair(zero?(4), 3) in b") 'int)

(check-exn exn:fail?
           (λ ()
             (check "unpair a b = 1 in a")))

(check-equal? (run "unpair a b = pair(7, 3) in a") (num-val 7))
(check-equal? (run "unpair a b = pair(7, 3) in b") (num-val 3))
...
(check-equal? (check "cons(1, emptylist)") '(listof int))
(check-equal? (check "cons(zero?(0), emptylist)") '(listof bool))
(check-equal? (check "cons(6, list(1, 4, 5))") '(listof int))
(check-equal? (check "null?(emptylist)") 'bool)
(check-equal? (check "null?(emptylist)") 'bool)
...
(check-equal? (run "cons(1, emptylist)") (list-val (num-val 1) (emptylist-val)))
(check-equal? (run "cons(zero?(0), emptylist)") (list-val (bool-val #t) (emptylist-val)))
...
(check-equal? (run "emptylist") (emptylist-val))
(check-equal? (run "emptylist") (emptylist-val))
...
(check-equal? (check "let f = proc (x : ?)
                                x
                      in if (f zero?(0))
                         then (f 11)
                         else (f 22)")
...
(check-equal? (check "letrec ? map (f : ?) = letrec ? foo (x : ?) = if null?(x)
                                                                    then emptylist
                                                                    else cons((f car(x)),
                                                                              ((map f) cdr(x)))
                                             in foo
                      in letrec ? even (y : ?) = if zero?(y)
                         ...
                         in pair(((map proc (x : int)
                                         -(x, 1))
                                  cons(3, cons(5, emptylist))),
                                 ((map even)
                                  cons(3, cons(5, emptylist))))")
              '(pairof (listof int) * (listof bool)))
