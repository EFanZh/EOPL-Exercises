...
;; Exercise 5.19 [★] Instead of placing the (lambda () ...) around the body of apply-procedure/k, place it around the
;; body of apply-cont. Modify the contracts to match this change. Does the definition of Bounce need to change? Then
;; replace the procedural representation of Bounce with a data-structure representation, as in exercise 5.18.
...
(define-datatype bounce bounce?
  [expval-bounce [val expval?]]
  [thunk-bounce [thunk procedure?]])
...
(define trampoline
  (lambda (bounce1)
    (cases bounce bounce1
      ...
      [thunk-bounce (thunk) (trampoline (thunk))])))
