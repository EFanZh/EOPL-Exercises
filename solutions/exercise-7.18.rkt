...
(define apply-subst-to-type
  (lambda (ty subst)
    (let loop ([ty ty]
               [cache '()]
               [cont (lambda (new-ty new-cache)
                       new-ty)])
      (cases type ty
        [int-type () (cont (int-type) cache)]
        [bool-type () (cont (bool-type) cache)]
        [proc-type (t1 t2) (loop t1
                                 cache
                                 (lambda (new-ty1 new-cache1)
                                   (loop t2
                                         new-cache1
                                         (lambda (new-ty2 new-cache2)
                                           (cont (proc-type new-ty1 new-ty2) new-cache2)))))]
        [tvar-type (sn) (let ([tmp (assoc ty cache)])
                          (if tmp
                              (cdr tmp)
                              (let ([tmp (assoc ty subst)])
                                (if tmp
                                    (loop (cdr tmp)
                                          cache
                                          (lambda (new-ty new-cache)
                                            (cont new-ty (cons (cons ty new-ty) new-cache))))
                                    (cont ty (cons (cons ty ty) cache))))))]))))
