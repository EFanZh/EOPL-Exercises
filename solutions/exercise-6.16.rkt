  ...
  (lambda ()
    (cases proc the-proc
      [procedure (vars body saved-env)
                 (set! the-exp body)
                 (set! the-env (extend-env* vars the-args saved-env))
                 (value-of/k)])))
  ...
  (lambda ()
    (cases tfexp the-exp
      [simple-exp->exp (simple)
                       (set! the-val (value-of-simple-exp simple the-env))
                       (apply-cont)]
      [cps-let-exp (var rhs body) (let ([val (value-of-simple-exp rhs the-env)])
                                    (set! the-exp body)
                                    (set! the-env (extend-env* (list var) (list val) the-env))
                                    (value-of/k))]
      [cps-letrec-exp (p-names b-varss p-bodies letrec-body)
                      (set! the-exp letrec-body)
                      (set! the-env (extend-env-rec** p-names b-varss p-bodies the-env))
                      (value-of/k)]
      [cps-if-exp (simple1 body1 body2) (if (expval->bool (value-of-simple-exp simple1 the-env))
                                            (begin (set! the-exp body1)
                                                   (value-of/k))
                                            (begin (set! the-exp body2)
                                                   (value-of/k)))]
      [cps-call-exp (rator rands) (let ([rator-proc (expval->proc (value-of-simple-exp rator the-env))]
                                        [rand-vals (map (lambda (simple)
                                                          (value-of-simple-exp simple the-env))
                                                        rands)])
                                    (set! the-proc rator-proc)
                                    (set! the-args rand-vals)
                                    (apply-procedure/k))])))
