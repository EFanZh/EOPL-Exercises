  ...
  (lambda (val cont)
    (let loop ([cont1 cont])
      (cases continuation cont1
        [try-cont (var cont-var handler-exp saved-env saved-cont) (value-of/k handler-exp
                                                                              (extend-env cont-var
                                                                                          (cont-val cont)
                                                                                          (extend-env var
                                                                                                      val
        ...
        [diff1-cont (exp2 saved-env saved-cont) (loop saved-cont)]
        [diff2-cont (val1 saved-cont) (loop saved-cont)]
        [if-test-cont (exp2 exp3 env saved-cont) (loop saved-cont)]
        [unop-arg-cont (unop saved-cont) (loop saved-cont)]
        [rator-cont (rand saved-env saved-cont) (loop saved-cont)]
        [rand-cont (val1 saved-cont) (loop saved-cont)]
        [raise1-cont (cont) (loop cont)]))))
...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
      ...
      [rand-cont (val1 saved-cont) (cases expval val1
                                     [proc-val (proc) (apply-procedure proc val saved-cont)]
                                     [cont-val (cont1) (apply-cont cont1 val)]
                                     [else (eopl:error 'apply-cont "expect procedure or continuation!")])]
      [try-cont (var cont-var handler-exp saved-env saved-cont) (apply-cont saved-cont val)]
  ...
  (lambda (exp env cont)
    (cases expression exp
      ...
      [letrec-exp (p-name b-var p-body letrec-body) (value-of/k letrec-body
                                                                ...
                                                                cont)]
      [try-exp (exp1 var cont-var handler-exp) (value-of/k exp1 env (try-cont var cont-var handler-exp env cont))]
