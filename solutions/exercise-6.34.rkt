...
;; Exercise 6.34 [★★] When we convert a program to CPS, we do more than produce a program in which the control
;; contexts become explicit. We also choose the exact order in which the operations are done, and choose names for each
...
(define cps-out-grammar
  '([cps-out-program (tfexp) cps-a-program]
    ...
    [tfexp ("let" identifier "=" tfexp "in" tfexp) cps-let-exp]
...
(define cps-of-zero?-exp
  (lambda (exp1)
    (cps-of-exps (list exp1)
                 (lambda (new-rands)
                   (simple-exp->exp (cps-zero?-exp (car new-rands)))))))

(define cps-of-sum-exp
  (lambda (exps)
    (cps-of-exps exps
                 (lambda (new-rands)
                   (simple-exp->exp (cps-sum-exp new-rands))))))

(define cps-of-diff-exp
  (lambda (exp1 exp2)
    (cps-of-exps (list exp1 exp2)
                 (lambda (new-rands)
                   (simple-exp->exp (cps-diff-exp (car new-rands) (cadr new-rands)))))))

(define cps-of-if-exp
  (lambda (exp1 exp2 exp3)
    (cps-of-exps (list exp1)
                 (lambda (new-rands)
                   (cps-if-exp (car new-rands) (cps-of-exp exp2) (cps-of-exp exp3))))))

(define cps-of-let-exp
  (lambda (id rhs body)
    (cps-let-exp id (cps-of-exp rhs) (cps-of-exp body))))

(define cps-of-letrec-exp
  (lambda (proc-names idss proc-bodies body)
    (cps-letrec-exp proc-names
                    idss
                    (map cps-of-exp proc-bodies)
                    (cps-of-exp body))))
...
(define cps-of-call-exp
  (lambda (rator rands)
    (cps-of-exps (cons rator rands)
                 (lambda (new-rands)
                   (cps-call-exp (car new-rands) (cdr new-rands))))))
...
(define cps-of-simple-exp
  (lambda (exp)
    (cases expression exp
      ...
      [proc-exp (ids exp) (cps-proc-exp ids (cps-of-exp exp))]
...
(define cps-of-exp
  (lambda (exp)
    (cases expression exp
      ...
      [proc-exp (vars body) (simple-exp->exp (cps-proc-exp vars (cps-of-exp body)))]
      [zero?-exp (exp1) (cps-of-zero?-exp exp1)]
      [diff-exp (exp1 exp2) (cps-of-diff-exp exp1 exp2)]
      [sum-exp (exps) (cps-of-sum-exp exps)]
      [if-exp (exp1 exp2 exp3) (cps-of-if-exp exp1 exp2 exp3)]
      [let-exp (var exp1 body) (cps-of-let-exp var exp1 body)]
      [letrec-exp (ids bidss proc-bodies body) (cps-of-letrec-exp ids bidss proc-bodies body)]
      [call-exp (rator rands) (cps-of-call-exp rator rands)])))
...
(define cps-of-program
  (lambda (pgm)
    (cases program pgm
      [a-program (exp1) (cps-a-program (cps-of-exp exp1))])))
...
(define value-of/k
  (lambda (exp env cont)
    (cases tfexp exp
      ...
      [cps-let-exp (var rhs body) (let ([val (value-of/k rhs env (end-cont))]) ; Special case for ANF program.
