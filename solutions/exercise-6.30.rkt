...
;;
;; For the use of cps-of-exps in cps-of-call-exp, we can use cps-of-exp/ctx on the rator, but we still need cps-of-exps
;; for the rands. Remove all other occurrences of cps-of-exps from the translator.
  ...
  (lambda (exp1 exp2 k-exp)
    (cps-of-exp/ctx exp1
                    (lambda (simp1)
                      (cps-of-exp/ctx exp2
                                      (lambda (simp2)
                                        (make-send-to-cont k-exp (cps-diff-exp simp1 simp2))))))))

(define cps-of-if-exp
  (lambda (exp1 exp2 exp3 k-exp)
    (cps-of-exp/ctx exp1
                    (lambda (simp1)
                      (cps-if-exp simp1 (cps-of-exp exp2 k-exp) (cps-of-exp exp3 k-exp))))))

(define cps-of-let-exp
  (lambda (id rhs body k-exp)
    (cps-of-exp/ctx rhs
                    (lambda (simp1)
                      (cps-let-exp id simp1 (cps-of-exp body k-exp))))))
...
(define cps-of-exp/ctx
  (lambda (exp context)
    (if (inp-exp-simple? exp)
        (context (cps-of-simple-exp exp))
        (let ([var (fresh-identifier 'var)])
          (cps-of-exp exp (cps-proc-exp (list var) (context (cps-var-exp var))))))))
...
(define cps-of-program
  (lambda (pgm)
    (cases program pgm
      [a-program (exp1) (cps-a-program (cps-of-exp/ctx exp1 simple-exp->exp))])))
