...
(define the-grammar
  '([program (expression) a-program]
    ...
    [type ("mutpair" type type) mutpair-type]
    [type ("void") void-type]))
...
(define expval->mutpair
  (lambda (v f)
    (cases expval v
      [mutpair-val (ref1 ref2) (f ref1 ref2)]
...
(define type-to-external-form
  (lambda (ty)
    (cases type ty
      ...
      [mutpair-type (ty1 ty2) (list 'mutpairof (type-to-external-form ty1) (type-to-external-form ty2))]
      [void-type () 'void])))
...
(define report-rator-not-a-mutpair-type
  (lambda (rator-type rator)
    (eopl:error 'type-of-expression
                "Rator not a mutpair type:~%~s~%had rator type ~s"
                rator
                (type-to-external-form rator-type))))
      ...
      [begin-exp (exp1 exps) (let loop ([exp1 exp1]
                               ...
                               (if (null? exps)
                                   ...
                                   (begin (type-of exp1 tenv)
                                          (loop (car exps)
                                                (cdr exps)))))]
      [newpair-exp (exp1 exp2) (mutpair-type (type-of exp1 tenv) (type-of exp2 tenv))]
      [left-exp (exp1) (let ([ty1 (type-of exp1 tenv)])
                         (cases type ty1
                           [mutpair-type (ty2 ty3) ty2]
                           [else (report-rator-not-a-mutpair-type ty1 exp1)]))]
      [setleft-exp (exp1 exp2) (let ([ty1 (type-of exp1 tenv)])
                                 (cases type ty1
                                   [mutpair-type (left-ty right-ty)
                                                 (check-equal-type! (type-of exp2 tenv) left-ty exp2)
                                                 (void-type)]
                                   [else (report-rator-not-a-mutpair-type ty1 exp1)]))]
      [right-exp (exp1) (let ([ty1 (type-of exp1 tenv)])
                          (cases type ty1
                            [mutpair-type (ty2 ty3) ty3]
                            [else (report-rator-not-a-mutpair-type ty1 exp1)]))]
      [setright-exp (exp1 exp2) (let ([ty1 (type-of exp1 tenv)])
                                  (cases type ty1
                                    [mutpair-type (left-ty right-ty)
                                                  (check-equal-type! (type-of exp2 tenv) right-ty exp2)
                                                  (void-type)]
                                    [else (report-rator-not-a-mutpair-type ty1 exp1)]))])))
      ...
      [begin-exp (exp1 exps) (let loop ([exp1 exp1]
                               ...
                               (if (null? exps)
                                   ...
                                   (begin (value-of exp1 env)
                                          (loop (car exps)
                                                (cdr exps)))))]
      [newpair-exp (exp1 exp2) (mutpair-val (newref (value-of exp1 env)) (newref (value-of exp2 env)))]
      [left-exp (exp1) (expval->mutpair (value-of exp1 env)
                                        (lambda (ref1 ref2)
                                          (deref ref1)))]
      [setleft-exp (exp1 exp2) (expval->mutpair (value-of exp1 env)
                                                (lambda (ref1 ref2)
                                                  (setref! ref1 (value-of exp2 env))))]
      [right-exp (exp1) (expval->mutpair (value-of exp1 env)
                                         (lambda (ref1 ref2)
                                           (deref ref2)))]
      [setright-exp (exp1 exp2) (expval->mutpair (value-of exp1 env)
                                                 (lambda (ref1 ref2)
                                                   (setref! ref2 (value-of exp2 env))))])))
