...
(define apply-thread
  (lambda (th)
    (cases thread th
      [cont-thread (cont val) (apply-cont cont val)]
      [proc-thread (proc arg) (apply-procedure proc arg (end-subthread-cont))])))
...
(define apply-cont
  (lambda (cont val)
    (if (time-expired?)
        (begin (place-on-ready-queue! (cont-thread cont val))
               (run-next-thread))
        (begin (decrement-timer!)
               (cases continuation cont
                 ...
                 [wait-cont (saved-cont) (wait-for-mutex (expval->mutex val) (cont-thread saved-cont (num-val 52)))]
