...
(define ref-type->base-type
  (lambda (ty)
    (cases type ty
      [ref-type (ty1) ty1]
      [else (eopl:error 'ref-type->base-type "Not a ref type: ~s" ty)])))
...
(define apply-subst-to-type
  (lambda (ty subst)
    (cases type ty
      ...
      [ref-type (ty1) (ref-type (apply-subst-to-type ty1 subst))]
      [void-type () (void-type)]
...
(define no-occurrence?
  (lambda (tvar ty)
    (cases type ty
      ...
      [ref-type (ty1) (no-occurrence? tvar ty1)]
      [void-type () #t]
      ...
      (cond [(equal? ty1 ty2) subst]
            ...
            [(and (proc-type? ty1) (proc-type? ty2)) (let ([subst (unifier (proc-type->arg-type ty1)
                                                       ...
                                                       (let ([subst (unifier (proc-type->result-type ty1)
                                                                             ...
                                                                             exp)])
                                                         subst))]
            [(and (ref-type? ty1) (ref-type? ty2)) (unifier (ref-type->base-type ty1)
                                                            (ref-type->base-type ty2)
                                                            subst
                                                            exp)]
...
(define type-of
  (lambda (exp tenv subst)
    (cases expression exp
      ...
      [begin-exp (exp1 exps) (let loop ([subst subst]
                                        [exp1 exp1]
                                        [exps exps])
                               (if (null? exps)
                                   (type-of exp1 tenv subst)
                                   (cases answer (type-of exp1 tenv subst)
                                     [an-answer (ty1 subst) (loop subst
                                                                  (car exps)
                                                                  (cdr exps))])))]
      [newref-exp (exp1) (cases answer (type-of exp1 tenv subst)
                           [an-answer (ty1 subst) (an-answer (ref-type ty1) subst)])]
      [deref-exp (exp1) (cases answer (type-of exp1 tenv subst)
                          [an-answer (ty1 subst) (let ([ty2 (fresh-tvar-type)])
                                                   (an-answer ty2 (unifier ty1 (ref-type ty2) subst exp1)))])]
      [setref-exp (exp1 exp2) (cases answer (type-of exp1 tenv subst)
                                [an-answer (ty1 subst) (cases answer (type-of exp2 tenv subst)
                                                         [an-answer (ty2 subst) (an-answer (void-type)
                                                                                           (unifier ty1
                                                                                                    (ref-type ty2)
                                                                                                    subst
                                                                                                    exp))])])])))
