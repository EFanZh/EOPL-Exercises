...
(define the-grammar
  '([program (expression) a-program]
    ...
    [expression ("letmut" (arbno identifier "=" expression) "in" expression) letmut-exp]
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      [letmut-exp (vars exps body) (let ([body-env (let loop ([vars vars]
                                                              [exps exps]
                                                              [env1 env])
                                                     (if (null? vars)
                                                         env1
      ...
      [assign-exp (var exp1) (let ([val (apply-env env var)])
                               (if (reference? val)
                                   (begin (setref! val (value-of exp1 env))
                                          (num-val 27))
                                   (eopl:error 'value-of "Variable ~s is immutable." var)))])))
