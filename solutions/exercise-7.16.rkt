...
;; Exercise 7.16 [★★★] Modify the grammar of INFERRED so that missing types are simply omitted, rather than marked
;; with ?.
...
(define the-grammar
  '([program (expression) a-program]
    ...
    [expression ("letrec" optional-type-2 identifier "(" identifier optional-type ")" "=" expression "in" expression)
                letrec-exp]
    [optional-type () no-type]
    [optional-type (":" type) a-type]
    [optional-type-2 () no-type-2]
    [optional-type-2 (type) a-type-2]
    [type ("int") int-type]
    [type ("bool") bool-type]
