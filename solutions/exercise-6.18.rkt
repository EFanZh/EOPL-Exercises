...
;; Exercise 6.18 [★★] Modify the grammar of CPS-OUT so that a simple diff-exp or zero?-exp can have only a constant or
;; variable as an argument. Thus in the resulting language value-of-simple-exp can be made nonrecursive.
...
(define cps-out-grammar
  '([cps-out-program (tfexp) cps-a-program]
    ...
    [simple-expression ("-" "(" atom-expression "," atom-expression ")") cps-diff-exp]
    [simple-expression ("zero?" "(" atom-expression ")") cps-zero?-exp]
    [simple-expression ("+" "(" (separated-list atom-expression ",") ")") cps-sum-exp]
...
(define inp-exp-simple?
  (lambda (exp)
    (cases expression exp
      [const-exp (num) #t]
      [var-exp (var) #t]
      [diff-exp (exp1 exp2) (and (inp-exp-atom? exp1) (inp-exp-atom? exp2))]
      [zero?-exp (exp1) (inp-exp-atom? exp1)]
      [proc-exp (ids exp) #t]
      [sum-exp (exps) (all-atom? exps)]
      [else #f])))
...
(define make-send-atom-to-cont
  (lambda (cont bexp)
    (let ([var (fresh-identifier 'var)])
      (cps-call-exp (cps-proc-exp '(k%00) (cps-call-exp (cps-atom-exp (cps-var-exp 'k%00)) (list bexp)))
                    (list (cps-proc-exp (list var) (make-send-to-cont cont (cps-atom-exp (cps-var-exp var)))))))))

(define cps-of-zero?-exp
  (lambda (exp1 k-exp)
    (cps-of-exps-to-atoms (list exp1)
                          (lambda (new-rands)
                            (make-send-to-cont k-exp (cps-zero?-exp (car new-rands)))))))

(define cps-of-zero?-exp-to-atom
  (lambda (exp1 k-exp)
    (cps-of-exps-to-atoms (list exp1)
                          (lambda (new-rands)
                            (make-send-atom-to-cont k-exp (cps-zero?-exp (car new-rands)))))))

(define cps-of-sum-exp
  (lambda (exps k-exp)
    (cps-of-exps-to-atoms exps
                          (lambda (new-rands)
                            (make-send-to-cont k-exp (cps-sum-exp new-rands))))))

(define cps-of-sum-exp-to-atom
  (lambda (exps k-exp)
    (cps-of-exps-to-atoms exps
                          (lambda (new-rands)
                            (make-send-atom-to-cont k-exp (cps-sum-exp new-rands))))))

(define cps-of-diff-exp
  (lambda (exp1 exp2 k-exp)
    (cps-of-exps-to-atoms (list exp1 exp2)
                          (lambda (new-rands)
                            (make-send-to-cont k-exp (cps-diff-exp (car new-rands) (cadr new-rands)))))))

(define cps-of-diff-exp-to-atom
  (lambda (exp1 exp2 k-exp)
    (cps-of-exps-to-atoms (list exp1 exp2)
                          (lambda (new-rands)
                            (make-send-atom-to-cont k-exp (cps-diff-exp (car new-rands) (cadr new-rands)))))))
...
(define cps-of-if-exp-to-atom
  (lambda (exp1 exp2 exp3 k-exp)
    (cps-of-exps (list exp1)
                 (lambda (new-rands)
                   (cps-if-exp (car new-rands) (cps-of-exp-to-atom exp2 k-exp) (cps-of-exp-to-atom exp3 k-exp))))))
...
(define cps-of-let-exp-to-atom
  (lambda (id rhs body k-exp)
    (cps-of-exps (list rhs)
                 (lambda (new-rands)
                   (cps-let-exp id (car new-rands) (cps-of-exp-to-atom body k-exp))))))
  ...
  (lambda (proc-names idss proc-bodies body k-exp)
    (cps-letrec-exp proc-names
                    (map (lambda (ids)
                         ...
                         idss)
                    (map (lambda (exp)
                           (cps-of-exp exp (cps-atom-exp (cps-var-exp 'k%00))))
                         proc-bodies)
                    (cps-of-exp body k-exp))))

(define cps-of-letrec-exp-to-atom
  (lambda (proc-names idss proc-bodies body k-exp)
    (cps-letrec-exp proc-names
                    (map (lambda (ids)
                           (append ids (list 'k%00)))
...
(define report-invalid-exp-to-cps-of-atom-exp
  (lambda (exp)
    (eopl:error 'cps-of-atom-exp "non-atom expression to cps-of-atom-exp: ~s" exp)))

(define cps-of-atom-exp
  (lambda (exp)
    (cases expression exp
      [const-exp (num) (cps-const-exp num)]
      [var-exp (var) (cps-var-exp var)]
      [else (report-invalid-exp-to-cps-of-atom-exp exp)])))
...
(define cps-of-simple-exp
  (lambda (exp)
    (cases expression exp
      [const-exp (num) (cps-atom-exp (cps-const-exp num))]
      [var-exp (var) (cps-atom-exp (cps-var-exp var))]
      [diff-exp (exp1 exp2) (cps-diff-exp (cps-of-atom-exp exp1) (cps-of-atom-exp exp2))]
      [zero?-exp (exp1) (cps-zero?-exp (cps-of-atom-exp exp1))]
      [proc-exp (ids exp) (cps-proc-exp (append ids (list 'k%00)) (cps-of-exp exp (cps-atom-exp (cps-var-exp 'k%00))))]
      [sum-exp (exps) (cps-sum-exp (map cps-of-atom-exp exps))]
      [else (report-invalid-exp-to-cps-of-simple-exp exp)])))

(define cps-of-exp-to-atom
  (lambda (exp cont)
    (cases expression exp
      [const-exp (num) (make-send-to-cont cont (cps-atom-exp (cps-const-exp num)))]
      [var-exp (var) (make-send-to-cont cont (cps-atom-exp (cps-var-exp var)))]
      [proc-exp (vars body) (make-send-atom-to-cont cont
                                                    (cps-proc-exp (append vars (list 'k%00))
                                                                  (cps-of-exp body (cps-var-exp 'k%00))))]
      [zero?-exp (exp1) (cps-of-zero?-exp-to-atom exp1 cont)]
      [diff-exp (exp1 exp2) (cps-of-diff-exp-to-atom exp1 exp2 cont)]
      [sum-exp (exps) (cps-of-sum-exp-to-atom exps cont)]
      [if-exp (exp1 exp2 exp3) (cps-of-if-exp-to-atom exp1 exp2 exp3 cont)]
      [let-exp (var exp1 body) (cps-of-let-exp-to-atom var exp1 body cont)]
      [letrec-exp (ids bidss proc-bodies body) (cps-of-letrec-exp-to-atom ids bidss proc-bodies body cont)]
      [call-exp (rator rands) (cps-of-call-exp rator rands cont)])))

(define cps-of-exps-to-atoms
  (lambda (exps builder)
    (define list-index
      (lambda (pred lst)
        (cond [(null? lst) #f]
    ...
    (let cps-of-rest ([exps exps])
      (let ([pos (list-index (lambda (exp)
                               (not (inp-exp-atom? exp)))
                             exps)])
        (if (not pos)
            (builder (map cps-of-atom-exp exps))
            (let ([var (fresh-identifier 'var)])
              (cps-of-exp-to-atom (list-ref exps pos)
  ...
  (lambda (exp cont)
    (cases expression exp
      ...
      [proc-exp (vars body) (make-send-to-cont cont
                                               (cps-proc-exp (append vars (list 'k%00))
                                                             (cps-of-exp body (cps-atom-exp (cps-var-exp 'k%00)))))]
...
(define value-of-simple-exp
  (lambda (exp env)
    (cases simple-expression exp
      ...
      [cps-diff-exp (exp1 exp2) (let ([val1 (expval->num (value-of-atom-exp exp1 env))]
                                      [val2 (expval->num (value-of-atom-exp exp2 env))])
                                  (num-val (- val1 val2)))]
      [cps-zero?-exp (exp1) (bool-val (zero? (expval->num (value-of-atom-exp exp1 env))))]
      [cps-sum-exp (exps) (let ([nums (map (lambda (exp)
                                             (expval->num (value-of-atom-exp exp env)))
                                           exps)])
