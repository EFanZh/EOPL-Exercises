...
;; Exercise 5.43 [★★] Modify letcc as defined in the preceding exercise so that the captured continuation becomes a new
;; kind of procedure, so instead of writing throw exp1 to exp2, one would write (exp2 exp1).
  ...
  (lambda (cont val)
    (cases continuation cont
      ...
      [rator-cont (rand saved-env saved-cont saved-try-cont) (value-of/k rand
                                                                         ...
                                                                         saved-try-cont)]
      [rand-cont (val1 saved-cont saved-try-cont) (cases expval val1
                                                    [proc-val (proc) (apply-procedure proc
                                                                                      val
                                                                                      saved-cont
                                                                                      saved-try-cont)]
                                                    [cont-val (cont) (apply-cont cont val)]
                                                    [else (eopl:error 'apply-cont
                                                                      "expect a procedure or a continuation")])]
